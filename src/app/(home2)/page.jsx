...
export default function Home2Page() {
  ...
  return (
    <div className="bg-background text-foreground min-h-screen font-sans selection:bg-amber-400 selection:text-slate-950 transition-colors duration-300">
      ...
      <section className="relative h-screen w-full overflow-hidden flex items-center justify-center pt-20">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-white via-white/40 to-white/70 dark:from-slate-950 dark:via-slate-950/40 dark:to-slate-950/70 z-10 transition-colors duration-300"></div>
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.15),rgba(255,255,255,0.85))] dark:bg-[radial-gradient(ellipse_at_center,rgba(12,16,21,0.1),rgba(12,16,21,0.85))] z-10 transition-colors duration-300"></div>

        {/* Content Container */}
        <div className="relative z-20 max-w-5xl mx-auto text-center px-6 flex flex-col items-center">
          
          {/* Micro-badge */}
          <div className="mb-6 flex items-center gap-2 px-4 py-1.5 bg-slate-200/50 dark:bg-white/5 border border-slate-350 dark:border-white/10 rounded-full backdrop-blur-md animate-luxury-float">
          ...
          <h1 className="text-5xl sm:text-7xl md:text-8xl font-light tracking-tight leading-none mb-6">
            <span className="block text-slate-800 dark:text-white/95 font-light">The</span>
          ...
          {/* Elegant Subtitle */}
          <p className="max-w-2xl text-base sm:text-xl text-slate-700 dark:text-white/70 font-light leading-relaxed mb-10 drop-shadow-md">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center z-30">
            ...
            <button
              ...
              className="cursor-pointer group relative inline-flex items-center justify-center gap-3 font-semibold px-8 py-4 text-base rounded-full transition-all duration-300 ease-in-out transform hover:scale-[1.03] active:scale-95 bg-slate-900 text-white dark:bg-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-emerald-400 shadow-xl"
            >
              <PlayCircle className="w-5 h-5 text-white dark:text-slate-950 transition-transform group-hover:rotate-12" />
            ...
            <button
              ...
              className="cursor-pointer group relative inline-flex items-center justify-center gap-2 font-semibold px-7 py-3.5 text-base rounded-full transition-all duration-300 ease-in-out transform hover:scale-[1.03] active:scale-95 bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-slate-800 dark:text-white border border-slate-300 dark:border-white/20 dark:hover:border-white/40 backdrop-blur-md"
            >
              <Heart className="w-4.5 h-4.5 transition-all text-rose-500 dark:text-rose-400 group-hover:scale-110" />
              <span>Donate</span>
            </button>
            ...
            {!authLoading && (!user || user.role === 'VIEWER') && (
              <button
                onClick={() => router.push(!user ? '/signup' : '/submit')}
                className="cursor-pointer group relative inline-flex items-center justify-center gap-2 font-semibold px-7 py-3.5 text-base rounded-full transition-all duration-300 ease-in-out transform hover:scale-[1.03] active:scale-95 bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-slate-800 dark:text-white border border-slate-300 dark:border-white/20 dark:hover:border-white/40 backdrop-blur-md"
              >
                <Users className="w-4.5 h-4.5 transition-all text-emerald-500 dark:text-emerald-400 group-hover:scale-110" />
          ...
          >
            <span className="text-[10px] uppercase tracking-widest text-slate-500 dark:text-white/50">Scroll to Begin</span>
            <ChevronDown className="w-4 h-4 text-emerald-500 dark:text-emerald-400 animate-bounce" />
          </div>
      ...
      <section className="relative py-28 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-20">
          ...
          <h2 className="text-3xl sm:text-5xl font-light tracking-tight text-foreground mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {/* Card 1 */}
          <div className="group relative p-8 bg-slate-100/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-3xl hover:border-emerald-400/30 hover:bg-slate-100 dark:hover:bg-white/[0.04] transition-all duration-300 flex flex-col justify-between">
          ...
          {/* Card 2 */}
          <div className="group relative p-8 bg-slate-100/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-3xl hover:border-amber-400/30 hover:bg-slate-100 dark:hover:bg-white/[0.04] transition-all duration-300 flex flex-col justify-between">
            <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-400/5 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <div>
      ...
      <section className="border-y border-slate-200 dark:border-white/5 bg-slate-100/30 dark:bg-white/[0.01] py-16 px-6 transition-colors duration-300">
      ...
      <section className="relative py-28 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center border border-slate-200 dark:border-white/10 rounded-[3rem] p-12 sm:p-20 bg-slate-100/30 dark:bg-white/[0.01] backdrop-blur-md relative z-10 transition-colors duration-300">
          <span className="text-xs font-bold uppercase tracking-widest text-emerald-400 block mb-3">Join the Mission</span>
          <h2 className="text-3xl sm:text-5xl font-light tracking-tight text-foreground mb-6 leading-tight">
          ...
          <p className="text-sm sm:text-base text-muted-foreground font-light leading-relaxed max-w-xl mx-auto mb-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
            <Link
               href="/submit"
               className="w-full sm:w-auto text-center bg-emerald-400 hover:bg-emerald-300 text-slate-950 font-bold px-8 py-3.5 rounded-full transition-colors duration-200 shadow-lg shadow-emerald-450/10"
            >
              Submit an Item
            </Link>
            <Link
              href="/contact"
              className="w-full sm:w-auto text-center bg-slate-200/50 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-slate-800 dark:text-white border border-slate-300 dark:border-white/15 px-8 py-3.5 rounded-full transition-colors duration-200"
            >
              Contact Curators
            </Link>
          </div>
